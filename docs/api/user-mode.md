...
calls `SYSCALL_TEST` to log the byte count. It then loops. The same program can
be pointed at the guard page to demonstrate the invalid-buffer error path.
This is a controlled ABI demonstration rather than a general process
lifecycle.
